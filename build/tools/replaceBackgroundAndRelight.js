...
const ReplaceBackgroundAndRelightArgsSchema = z
    ...
    message: "Either backgroundPrompt or backgroundReferenceUri must be provided",
});
export const replaceBackgroundAndRelightToolDefinition = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            outputImageFileName: {
                type: "string",
                description: "The desired name of the output image file, no file extension. Make it descriptive but short. Lowercase, dash-separated, no special characters.",
            },
        },
        required: ["imageFileUri", "outputImageFileName"],
    },
};
export const replaceBackgroundAndRelight = async (args, context) => {
    const validatedArgs = ReplaceBackgroundAndRelightArgsSchema.parse(args);
    const resourceClient = getResourceClient();
    const imageFilePath = await resourceClient.resourceToFile(validatedArgs.imageFileUri, context);
    ...
    if (validatedArgs.backgroundReferenceUri) {
        backgroundReference = await resourceClient.resourceToFile(validatedArgs.backgroundReferenceUri);
    }
    let lightReference;
    if (validatedArgs.lightReferenceUri) {
        lightReference = await resourceClient.resourceToFile(validatedArgs.lightReferenceUri);
    }
